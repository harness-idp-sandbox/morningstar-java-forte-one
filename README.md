...
# run app
java -jar target/java-forte-one-1.0-SNAPSHOT.jar
...
docker buildx build   --platform linux/amd64   -t parsontodd/java-forte-one:latest   --push .
```
...
# watch rollout
kubectl rollout status deploy/java-forte-one -n dev

# get external endpoint
kubectl get svc java-forte-one-svc -n dev
```
...
```bash
kubectl port-forward svc/java-forte-one-svc 8080:80 -n dev
...
kubectl rollout restart deploy/java-forte-one -n dev
kubectl rollout status deploy/java-forte-one -n dev
```
...
│  │        └─ index.html
│  └─ test/
│     └─ java/com/harness/javaforteone/AppTest.java
├─ kubernetes/
│  ├─ deployment.yml
│  └─ service.yml
├─ Dockerfile
...
© alexandro.forte@morningstar.com. For demo/PoV purposes.
